// This file contain initial implementation of dither
...
int main() {
  ...
  unsigned int err =
      loadbmp_decode_file("in3.bmp", &pixelsIn, &width, &height, LOADBMP_RGBA);
  ...
  for (int i = 0; i < height; ++i) {
    for (int j = 0; j < width; ++j) {
      ...
      rBuf[i % kernelHeight][j] = gBuf[i % kernelHeight][j] =
          bBuf[i % kernelHeight][j] = 0;
      ...
      int rErr =
          r -
          (int)(*ptrOut++ = pallete[(i % 3 + 2 - j % 3) % 3][mi] >> 16 & 0xFF);
      int gErr =
          g -
          (int)(*ptrOut++ = pallete[(i % 3 + 2 - j % 3) % 3][mi] >> 8 & 0xFF);
      int bErr =
          b -
          (int)(*ptrOut++ = pallete[(i % 3 + 2 - j % 3) % 3][mi] >> 0 & 0xFF);
      *ptrOut++ = 0xFF;
